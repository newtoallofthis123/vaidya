...
const AudioRecorder = () => {
    ...
    const sendAudioToServer = async (audioBlob) => {
        try {
            ...
            const response = await fetch("http://192.168.0.127:8000/transcribe", {
                method: "POST",
                body: formData,
            });
            ...
            !isRecording ? (
                !message ? (
                    <div>
                        ...
                        }} transition={{ bounce: 0.2, duration: 1.5 }} className="text-3xl font-bold text-center">
                            Please state your general information.
                        </motion.h1>
                        <motion.p initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 1 }} className="text-lg pt-2 text-center">
                            Include your name, address, and contact information.

                        </motion.p>
                        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 2 }} className="text-center mt-4">
                            <motion.div
